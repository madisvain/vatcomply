...
@app.route("/rates/latest", methods=["GET", "HEAD"])
@app.route("/rates/{date}", methods=["GET", "HEAD"])
...
@app.route("/rates/history", methods=["GET", "HEAD"])
async def rates(request):
    query_params = dict(request.query_params)
    ...
    if "start_date" not in query_params:
        return UJSONResponse(
            {"error": "Missing start_date query parameter"}, status_code=400
        )

    # End date
    if "end_date" not in query_params:
        return UJSONResponse(
            {"error": "Missing end_date query parameter"}, status_code=400
        )
